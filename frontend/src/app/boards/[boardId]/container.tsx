...
function BoardDetailContent({ boardId }: { boardId: string }) {
  ...
  return (
    <>
      {isLoading ? (
        <main className="flex flex-col overflow-hidden px-6 py-6">
          <BoardDetailSkeleton />
        </main>
      ) : isError || !board ? (
        ...
        <div className="flex  flex-col">
          <HeaderSlot>
            <div className="hidden min-w-0 items-center gap-2 border-l border-border pl-6 sm:flex">
              <Link
                href="/boards"
              ...
              <span className="min-w-0 max-w-xs truncate font-medium text-foreground">
                {board.name}
              </span>
              {board.myRole === "VIEWER" && (
